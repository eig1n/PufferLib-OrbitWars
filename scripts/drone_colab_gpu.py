#!/usr/bin/env python3
"""
Drone Colab GPU runner.
Assumes colab_setup.py has been run.
Pulls latest changes, builds the CUDA backend for drone, runs a short training job for 1 minute,
...
log_file = os.path.join(log_dir, "drone_colab_gpu.log")
sys.stdout = Tee(log_file)
sys.stderr = sys.stdout

print("=" * 72)
print("DRONE ENVIRONMENT - COLAB GPU RUN")
...
# Build CUDA for drone
run(["bash", "build.sh", "drone"], cwd=REPO_DIR, env=env)
...
print(f"CONSOLE_LOG_FILE=/content/pufferlib/logs/drone_colab_gpu.log")
print("=" * 72)
